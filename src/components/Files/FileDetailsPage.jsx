  ...
  const FileDetailsPage = () => {
    ...
    useEffect(() => {
      if (reportId) {
        ...
        setError("نمی تواند شناسه گزارش را بازیابی کند.");
      }
    }, [reportId]);
    ...
    const fetchDocuments = async (reportId) => {
      try {
        ...
        setError('دریافت اسناد انجام نشد. لطفا دوباره تلاش کنید.');
    ...
    const handleFileUpload = async () => {
      ...
      try {
        ...
        setError('فایل آپلود نشد. لطفا دوباره تلاش کنید.');
        ...
        <div className="flex flex-col w-full mt-8 px-5 max-md:mt-10 max-md:max-w-full m-[-100px]" dir="rtl">
          ...
          <div className="mb-3 w-full max-w-md">
            ...
            <select id="documentType" onChange={(e) => setSelectedDocumentType(e.target.value)} className="block w-full bg-white border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500">
              <option value=""></option>
              {documents.map((doc) => (
                <option key={doc._id} value={doc.document}>
                  {doc.document_fa || doc.document}
                </option>
              ))}
            </select>
          </div>
